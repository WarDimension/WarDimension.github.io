...
(function() {
    ...
    const conf = {
        ...
        "autoCompleteLineOnEnter": false, // autocomplete with Enter without pressing Shift (caret has to be at the very end)
    ...
    style.innerHTML = `
        ...
        .enter span, .tab-enter span, .target-enter span{
            position: absolute;
            color: transparent;
        }
        .incorrect{
    ...
    typingTarget.appendChild(typingInput);
    ...
    zenMode.addEventListener("click", () => {
        ...
        if(textElements.length > 0){
            ...
            setText("にゃにゃめにゃにゃじゅうにゃにゃどのにゃらびでにゃくにゃくいにゃにゃくにゃにゃはんにゃにゃだいにゃんにゃくにゃらべてにゃがにゃがめ");
        }
    ...
    function setText(text){
        const stringSplit = text.replaceAll("\t", "").split("\n");

        for(let i = 0; i < stringSplit.length; i++){
            const newDiv = document.createElement("div");
            newDiv.id = "line-" + i;
            newDiv.className = "lines";
            ...
            const targetHidden = document.createElement("span");
            targetHidden.className = "target-hidden";

            const targetVisible = document.createElement("span");
            targetVisible.className = "target-visible";
            targetVisible.innerText = stringSplit[i];

            target.appendChild(targetHidden);
            target.appendChild(targetVisible);
            newDiv.appendChild(check);
            newDiv.appendChild(target);

            if(stringSplit[i + 1] !== undefined){
                const newSpan = document.createElement("span");
                newSpan.id = "target-enter-" + i;
                newSpan.className = "target-enter";

                const enter = document.createElement("span");
                enter.innerText = "↵";

                newSpan.appendChild(enter);
                newDiv.appendChild(newSpan);
            }

            typingTarget.appendChild(newDiv);
        }
    }
    ...
    function updateTypingCheck(e){
        const typingInputTextSplit = typingInput.value.split("\n");
        const lines = typingTarget.querySelectorAll(".lines");

        for(let i = 0; i < lines.length; i++){
            const check = lines[i].querySelector(".check");
            const targetHidden = lines[i].querySelector(".target-hidden");
            const targetVisible = lines[i].querySelector(".target-visible");
            const targetText = targetHidden.textContent + targetVisible.textContent;

            const typingInputLength = typingInputTextSplit[i] != undefined ? typingInputTextSplit[i].length : 0;

            let length = Math.abs(typingInputLength - check.textContent.length);

            if(check.textContent.length > targetText.length) length -= check.textContent.length - targetText.length;

            if(typingInputLength > check.textContent.length){
                targetHidden.textContent += targetVisible.textContent.slice(0, length);
                targetVisible.textContent = targetVisible.textContent.slice(length);
            }
            else if(typingInputLength < check.textContent.length){
                targetVisible.textContent = targetHidden.textContent.slice(targetHidden.textContent.length - length) + targetVisible.textContent;
                targetHidden.textContent = targetHidden.textContent.slice(0, targetHidden.textContent.length - length);
            }

            //check.textContent = typingInputTextSplit[i];
            updateCheck(check, targetText, typingInputTextSplit[i]);
        }
        /*
    ...
    function updateCheck(check, targetText, input){
        check.innerHTML = "";
        const inputLength = input != undefined ? input.length : 0;
        const length = Math.max(targetText.length, inputLength);

        let newSpan = document.createElement("span");
        check.appendChild(newSpan);

        for(let i = 0; i < length; i++){
            if(i < inputLength){
                if(targetText[i] != input[i]){
                    if(!newSpan.classList.contains("incorrect")){
                        if(i > 0) newSpan = document.createElement("span");
